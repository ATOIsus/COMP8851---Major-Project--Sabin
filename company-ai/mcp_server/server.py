...
def log_action(action, tool, input_summary, result, request: Request | None = None):
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?)""",
        (execution_id, email_id, action, tool, str(input_summary)[:2000], str(result)[:10000]),
    )
    conn.commit()
    conn.close()
...
def fetch_unread_emails(request: Request):
    ...
    with open(path, encoding="utf-8") as f:
        emails = json.load(f)
    log_action("fetch_unread_emails", "email", "sample mailbox", f"{len(emails)} emails", request)
...
def search_documents(req: SearchRequest, request: Request):
    ...
    log_action("search_documents", "file_search", req.query, f"{len(results)} matches", request)
...
def read_file(req: ReadFileRequest, request: Request):
    ...
    with open(path, "r", encoding="utf-8") as f:
        content = f.read()
    log_action("read_file", "filesystem", f"{req.location}/{req.filename}", f"{len(content)} chars", request)
...
def write_transaction_log(req: TransactionLogRequest, request: Request):
    ...
    log_action("write_transaction_log", "filesystem", req.entry[:50], "appended", request)
...
def cleanup_temp_attachments(request: Request):
    ...
    log_action("cleanup_temp_attachments", "filesystem", "periodic cleanup", deleted, request)
